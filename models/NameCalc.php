...
class NameCalc extends Model
{
    ...
    public static $name_types = ['父亲', '母亲', '丈夫', '妻子', '儿子', '女儿', '哥哥', '弟弟', '姐姐', '妹妹'];
     ...
     * @return array
     */
    public static function calculateName($params)
    {
        $options = [
            'text' => '',
            'sex' => -1,
        ...
        $selectors = self::getSelectors($options['text']);
        $result = [];
        foreach ($selectors as $selector) {
            $ids = self::selectorToId($selector, $options['sex']);
            foreach ($ids as $id) {
                if ($options['type'] == 'chain') {
                    $item = self::getChainById($id);
                    if ($item) {
                        $result[] = $item;
                    }
                } else {
                    if ($options['reverse']) {
                        $id = self::reverseId($id, $options['sex']);
                    }
                    $items = self::getDataById($id);
                    if ($items) {
                        $result = array_merge($result, $items);
                    } else if (strpos($id, 'w') === 0 || strpos($id, 'h') === 0) {
                        $items = self::getDataById(substr($id, 2));
                        if ($items) {
                            $result = array_merge($result, $items);
                        }
                    }
     ...
     * @return string
     */
    private static function getChainById($id)
    {
        $arr = explode(',', $id);
        $items = [];
        foreach ($arr as $a) {
            $key = preg_replace('/&[ol]/', '', $a, 1);
            $items[] = self::$relation_data[$key][0];
        }
     ...
     * @return string
     */
    private static function reverseId($id, $sex)
    {
        $hash = [
            'f' => ['d', 's'],
            'm' => ['d', 's'],
     ...
     * @return array
     */
    private static function getData($d)
    {
        $res = [];
        foreach (self::$relation_data as $key => $value) {
            if (preg_replace('/&[olx]/', '', $key) == $d) {
                $res[] = self::$relation_data[$key][0];
            }
        }
        return $res;
    }
    ...
    {
        $items = [];
        if (isset(self::$relation_data[$id])) {
            $items[] = self::$relation_data[$id][0];
        } else {
            $items = self::getData($id);
            if (!$items) {
                $id = preg_replace('/&[ol]/', '', $id);
                $items = self::getData($id);
            }
            ...
            if (!$items) {
                ...
                $items = self::getData($l);
                $o = preg_replace('/x/', 'o', $id);
                $items = array_merge($items, self::getData($o));
            }
        }
        return $items;
    }
     ...
     * @return array|bool
     */
    private static function selectorToId($selector, $sex)
    {
        $result = [];
        $hash = [];
        if ($sex < 0) {
        ...
        if (preg_match('/,[w0],w|,[h1],h/', $selector)) {
            return $result;
        }

        $getId = function ($selector) use (&$getId, &$result, &$hash) {
            if (!isset($hash[$selector])) {
                $hash[$selector] = true;
                $status = true;
                do {
                    $s = $selector;
                    foreach (NameCalc::$filters as $key => $value) {
                        $item = NameCalc::$filters[$key];
                        $selector = preg_replace($item['exp'], $item['str'], $selector, $item['limit']);
                ...
                if ($status) {
                    ...
                    $selector = substr(preg_replace('/,[01]/', '', $selector, 1), 1);
                    $result[] = $selector;
                }
            }
        };
    ...
    private static function unique($arr)
    {
        ...
        for ($i = 0; isset($arr[$i]); $i++) {
            $item = $arr[$i];
            $temp = preg_replace('/&[ol]/', '', preg_replace('/[ol](?=s|b)/', 'x', $item, 1), 1);
            if (!isset($hash[$item]) && !isset($hash[$temp])) {
                $result[] = $item;
                $hash[$item] = true;
            }
        }
     ...
     * @return array
     */
    private static function getSelectors($str)
    {
        $str = preg_replace('/[二|三|四|五|六|七|八|九|十]{1,2}/u', 'x', $str);
        $lists = preg_replace('/我/u', '', $str, 1);
        $lists = preg_replace('/家的/u', '的', $lists, 1);
        $lists = preg_replace('/家/u', '的', $lists, 1);
        $lists = explode('的', $lists);
        $result = [];
        $match = true;
        ...
        while ($lists) {
            ...
            $arr = [];
            $has = false;
            foreach (self::$relation_data as $i => $v) {
                $value = self::$relation_data[$i];
                if (array_search($name, $value) > -1) {
                    if ($i || !$lists) {
                        $arr[] = $i;
                    }
                    $has = true;
